...
export async function GET(req: Request, { params }: { params: { tag: string } }) {
  try {
    ...
    if (!p.ok) return NextResponse.json({ code: p.status, message: await p.text() || p.statusText }, { status: p.status });
    if (!b.ok) return NextResponse.json({ code: b.status, message: await b.text() || b.statusText }, { status: b.status });
